...
import { HTTP_STATUSES, HTTP_STATUSES_CODES } from '../../core/consts/http-error-statuses.js';
...
export function getUsers(resp: ServerResponse): void {
  ...
  resp.statusCode = HTTP_STATUSES_CODES[HTTP_STATUSES.OK_200];
...
export function createUser(data: CreateUserDto, resp: ServerResponse): void {
  ...
  if (isUserDataValid(data)) {
    resp.statusCode = HTTP_STATUSES_CODES[HTTP_STATUSES.CREATED_201];
...
export function removeUser(userId: string, resp: ServerResponse) {
  ...
  if (isSuccessfully) {
    resp.statusCode = HTTP_STATUSES_CODES[HTTP_STATUSES.NO_CONTENT_204];
    resp.end();
  } else {
    errorHandler(resp, HTTP_STATUSES.NOT_FOUND_404_User);
  }
